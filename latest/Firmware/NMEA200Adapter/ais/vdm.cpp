...
#ifdef STM32
    int32_t checksum = strtol(checksum_str.c_str(), nullptr, 16);
#else
...
#else
  *sentence_total = atoi(fields[1].c_str());
#endif
...
#else
  *sentence_number = atoi(fields[2].c_str());
#endif
...
#else
    *sequence_number = atoi(fields[3].c_str());
#endif
...
#else
  fill_bits = atoi(fields[6].substr(0,1).c_str());
#endif
